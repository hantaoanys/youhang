<template>
  <view class="rf-notice-detail">
    <view class="banner" v-if="announceDetail.cover">
      ...
      <view class="banner-title in2line">{{announceDetail.noticeName}}</view>
    </view>
    <view class="banner-title" v-if="!announceDetail.cover">{{announceDetail.noticeName}}</view>
    <view class="article-meta" v-if="announceDetail.createTime">
      <text class="article-author"><!-- {{announceDetail.author_name}} --> 系统管理员</text>
      <text class="article-text">发布于</text>
      <text class="article-time">{{announceDetail.createTime | time}}</text>
    </view>
    <view class="article-content">
      <rich-text :nodes="announceDetail.noticeDiscription"></rich-text>
    </view>
		...
		methods: {
			...
			async getNotifyAnnounceView(id) {
				await this.$http.get(`${notifyAnnounceView}`+"/"+id).then(r => {
					var title = r.data.data.noticeName;
					this.loading = false;
					this.announceDetail = r.data.data;
					uni.setNavigationBarTitle({
						title: r.data.data.noticeName
					});
				}).catch(() => {
					this.loading = false;
				})
