<template>
  <view class="product-list">
    ...
    <rf-empty :info="errorInfo || '该分暂无商品'" v-if="goodsList.length === 0 && !loading"></rf-empty>
    <!--分类遮盖层-->
    <!--页面加载动画-->
	...
	export default {
      ...
      // 跳转至主页
			...
			async getProductList(type) {
				await this.$http.get(`${productList}?name=`+this.sketch+'&keywords='+this.keywords, {
				}).then(async r => {
					this.loading = false;
					if (type === 'refresh'){
						uni.stopPullDownRefresh();
					}
					this.loadingType = 'nomore'; //没有更多了
					this.goodsList = [...this.goodsList, ...r.data.rows];
